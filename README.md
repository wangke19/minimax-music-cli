...
> ### 🧠 认知层面
>
> 本工具是 **AI 辅助创作** 而非全自动生产。你需要理解：
>
> - MiniMax API 是创作引擎，**你的 prompt 质量决定输出质量**
> - AI 生成的歌词和旋律可能存在风格偏差，**人工筛选和迭代是必要环节**
> - 证据链和版权报告是 AI 创作的可追溯性保障，**不是法律文件**
> - 免费额度每天 100 首，**合理规划批量规模**
> - `--use-lyrics-gen` 显著提升时长和质量，**推荐有声乐时始终使用**
...
> 掌握本工具后，你将能够：
>
> - **单曲精调**：通过 CLI 参数精确控制音乐风格、时长、音质、采样数
> - **批量生产**：从提示词库一键生成数十首音乐，支持并发加速
> - **质量控制**：利用 A/B 采样（`-s 2`）对比不同版本，精选最佳输出
> - **证据追溯**：通过 `chain.jsonl` 和版权报告追溯每首作品的完整创作过程
> - **断点续传**：批量任务中断后自动保存进度，随时可恢复
> - **自定义扩展**：基于模块化架构（api/generators/evidence）扩展新的生成策略
...
> | 场景 | 推荐做法 | 原因 |
> |------|----------|------|
> | 追求质量 | 单曲 `--use-lyrics-gen` | AI 歌词 + 完整结构，时长 190-230s |
> | 追求数量 | `batch_generate.py` 批量 | 一次性处理数十条 prompt |
> | 并发加速 | `-c 3`（免费）/ `-c 10`（付费） | 在限额内最大化吞吐 |
> | A/B 对比 | `-s 2` 搭配人工筛选 | 同 prompt 多版本择优 |
> | 纯音乐 | `--instrumental` 或 `纯音乐,` 前缀 | 无需人声的场景 |
> | 风格探索 | prompt 简洁（200-400 字） | 过长 prompt 导致风格稀释 |
> | 额度紧张 | 纯音乐模式 | 减少歌词 API 调用，节省配额 |
